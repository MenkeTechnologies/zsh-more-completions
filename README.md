...
- **Madagascar (seismic / RSF)** — `sfin`, `sfwindow`, `sfmath`, `sfcat`, `sfattr`, `sfreverse`, `sftransp`, `sfdd`, `sfpad`, `sfdisfil`, `sfheaderwindow`, `sfheadersort`, `sfheadercut`, `sfget`, `sfput`, `sfspike`, `sfcmplx`, `sfreal`/`sfimag`, `sfinterleave`, `sfmask` — key=value parameter style with names + types resolved from each tool's `sf_getbool()`/`sf_getint()`/`sf_getfloat()`/`sf_getstring()`/`sf_getfloats()`/`sf_getints()` calls in `ahay/src/system/main/*.c` (e.g. `spike.c nsp= mag= title= k# l# p#`, `interleave.c axis=`, `mask.c min= max=`)
