...
The completion corpus was sourced by harvesting tool names from every major package ecosystem and then generating/verifying completions for each binary. **Already harvested:**
...
- **Debian / Ubrunutu APT** — every `bin/*` from `apt-cache pkgnames`, `/usr/share/man/man1`, `/usr/share/man/man8`, `/usr/share/man/man5`, `/usr/share/man/man6`, etc.
- **Arch Linux pacman / AUR** — every binary from `pacman -Ql` across `core`/`extra`/`community` plus the AUR
- **GitHub Releases** — direct binary harvests across thousands of repos shipping prebuilt artifacts (Rust/Go/Node CLIs)
- **PyPI / pip** — every Python package that exposes a `console_scripts` entry point
- **npm** — every package with a `bin` field in its `package.json`
- **crates.io** — every Rust crate that produces a binary target
- **Go modules** — every importable `cmd/*` across the public module graph
- **RubyGems** — every gem with an `executables` entry in its gemspec
- **Hackage** — every Haskell package shipping an executable
- **OPAM** — every OCaml package with a `bin` artifact
- **CPAN** — every Perl distribution shipping an executable script
- **CRAN** — every R package shipping a CLI helper
- **Hex (Elixir/Erlang)** — every package with an escript or release binary
- **Chocolatey / Scoop / WinGet** — Windows CLI binaries with Linux/macOS equivalents
- **Conda-forge** — every package whose `entry_points` produce a CLI
- **Snap / Flatpak** — every confined CLI app
- **FreeBSD / OpenBSD / NetBSD ports** — every port with a `bin/*`
- **Alpine apk** — every package with a `bin/*`
- **Fedora / RHEL DNF / RPM** — every `bin/*` across Fedora, EPEL, and RPMFusion
- **macOS system binaries** — every executable in `/usr/bin`, `/usr/sbin`, `/usr/libexec`, `/System/Library/CoreServices`, `/Applications/*.app/Contents/MacOS`
- **JetBrains Toolbox CLIs**, **GitLab/Bitbucket project releases**, **HuggingFace CLIs**, **vendor SDKs** (AWS, GCP, Azure, Oracle, IBM, NVIDIA, etc.)

Any binary that shipped via any of the channels above and was reachable through `--help`, a man page, or a documented CLI reference has had a completion file generated, syntax-checked, and committed. **Verification is the ongoing work** — every file is being read against its real upstream documentation, replacing the auto-generated stub with a source-cited completion and removing fabricated flags.
