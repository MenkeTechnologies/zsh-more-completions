...
A **massive** zsh completions plugin — **~13,626 commands** wired into the compsys neural net. Most were auto-generated by python scripts that jacked into `--help` output and man pages, ripping completion data straight from the source. Quality varies — some are razor-sharp, some are rough chrome. Architecture-prefixed completions live in the `architecture_src` directory.

```
src/              -> main completions
man_src/          -> completions parsed from man pages
override_src/     -> completions that override defaults (prepended to fpath)
architecture_src/ -> cross-architecture completions
```
