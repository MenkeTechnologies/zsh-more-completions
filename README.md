...
- **Madagascar (seismic / RSF)** — `sfin`, `sfwindow`, `sfmath`, `sfcat`, `sfattr`, `sfreverse`, `sftransp`, `sfdd`, `sfpad`, `sfdisfil` — key=value parameter style with names + types resolved from each tool's `sf_getbool()`/`sf_getint()`/`sf_getfloat()`/`sf_getstring()` calls in `ahay/src/system/main/*.c` (e.g. `in.c info= check= trail=`, `window.c verb= squeeze= n# f# j# d# min# max#`, `attr.c want= lval=`, `reverse.c which= opt= memsize=`, `dd.c form= type= ibm= trunc=`, `pad.c beg# end# n#out`)
