...
- **Apache Jena (RDF / SPARQL)** — `riot` (RDF I/O Tool), `rsparql`/`sparql`/`arq`/`qparse` (SPARQL query), `shacl` (W3C SHACL validate + parse), `tdb2.tdbloader` (TDB2 bulk load), `fuseki-server` (Fuseki2 SPARQL HTTP server); flag tables harvested from `jena.apache.org/documentation/{io,query,shacl,tdb2,fuseki2}/` reference pages and `apache/jena/jena-fuseki2/jena-fuseki-main/` Java sources
