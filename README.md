...
- **OpenFOAM (CFD)** — mesh + solver + post-processing utilities (`checkMesh`, `potentialFoam`, `foamToVTK`, `setFields`, `topoSet`, plus existing `blockMesh`/`decomposePar`/`reconstructPar`/`pimpleFoam`/`simpleFoam`/`snappyHexMesh`); per-tool `argList::addOption()`/`addBoolOption()` strings + the common argList option set (`-case`, `-parallel`, `-roots`, `-region`, …) read from `OpenFOAM/OpenFOAM-12/applications/**/*.C` and `src/OpenFOAM/global/argList/argList.C`
