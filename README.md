...
- **Madagascar (seismic / RSF)** — `sfin`, `sfwindow`, `sfmath`, `sfcat`, `sfattr` — key=value parameter style with names + types resolved from each tool's `sf_getbool()`/`sf_getint()`/`sf_getfloat()`/`sf_getstring()` calls in `ahay/src/system/main/*.c` (e.g. `in.c info= check= trail=`, `window.c verb= squeeze= n# f# j# d# min# max#`, `attr.c want= lval=`)
