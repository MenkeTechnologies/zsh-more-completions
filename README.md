...
- **Gentoo portage-utils** (q-tools) — `qlist`, `qsearch`, `qfile`, `qdepends`, `qsize` (alongside existing `emerge`, `equery`, `eix`, `eselect`, `layman`); per-tool `qX_long_opts[]`/`qX_opts_help[]` static arrays read from each binary's source at `gentoo/portage-utils q*.c`. Notable: `qdepends -d/-r/-p/-b/-i` exposes the full DEPEND/RDEPEND/PDEPEND/BDEPEND/IDEPEND dependency atoms; `qsize -m/-k/-b` is modelled as a mutually-exclusive unit group.
