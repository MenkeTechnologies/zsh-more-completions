...
- **GMT** (Generic Mapping Tools, classic-mode) — `psxy`, `pscoast`, `grdimage`, `grdmath`, `grdtrack` (alongside existing `gmt` modern-mode wrapper); single-letter option syntax (`-J` projection, `-R` region, `-B` basemap, `-W` pen, `-G` fill, …) + per-program options (e.g. `pscoast -D{full,high,intermediate,low,crude}`, `grdtrack -S{mean,median,mode}` stack reducers, `grdmath` 200+ RPN operators) harvested from https://docs.generic-mapping-tools.org/latest/{plot,coast,grdimage,grdmath,grdtrack}.html
