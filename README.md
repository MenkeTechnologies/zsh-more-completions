...
- **GMT** (Generic Mapping Tools, classic-mode) — `psxy`, `pscoast`, `grdimage`, `grdmath`, `grdtrack`, `pstext`, `psbasemap`, `psconvert`, `grdfilter`, `grdsample` (alongside existing `gmt` modern-mode wrapper); single-letter option syntax (`-J` projection, `-R` region, `-B` basemap, `-W` pen, `-G` fill, …) + per-program options (e.g. `pscoast -D{full,high,intermediate,low,crude}`, `grdtrack -S{mean,median,mode}` stack reducers, `grdmath` 200+ RPN operators, `grdfilter -F{b,c,g,f,o,m,p,h,l,L,u,U}` filter types, `psconvert -T{b,e,f,j,g,G,m,s,t,F}` output formats) harvested from https://docs.generic-mapping-tools.org/latest/{plot,coast,grdimage,grdmath,grdtrack,text,basemap,psconvert,grdfilter,grdsample}.html
