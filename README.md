...
- **GMT** (Generic Mapping Tools, classic-mode) — 25 binaries: `psxy`, `pscoast`, `grdimage`, `grdmath`, `grdtrack`, `pstext`, `psbasemap`, `psconvert`, `grdfilter`, `grdsample`, `grdcontour`, `xyz2grd`, `grd2xyz`, `nearneighbor`, `blockmean` (shared compdef also serves `blockmedian`/`blockmode`), `mapproject`, `triangulate`, `makecpt`, `grdinfo`, `grdcut`, `grdedit`, `grdgradient`, `grdblend`, `gmtset`, `gmtwhich` (alongside existing `gmt` modern-mode wrapper); single-letter option syntax (`-J` projection, `-R` region, `-B` basemap, `-W` pen, `-G` fill, …) + per-program options (e.g. `pscoast -D{full,high,intermediate,low,crude}`, `grdtrack -S{mean,median,mode}` stack reducers, `grdmath` 200+ RPN operators, `grdfilter -F{b,c,g,f,o,m,p,h,l,L,u,U}` filter types, `psconvert -T{b,e,f,j,g,G,m,s,t,F}` output formats, `blockmean -S{m,n,s,w}` statistic, `makecpt -F{R,r,h,c,gray}` CPT output format, `grdcut -E{x,y}` 3-D cube slice axis, `grdblend -C{f,l,o,u}` clobber mode, `gmtwhich -G{a,c,l,u}` download destination) harvested from https://docs.generic-mapping-tools.org/latest/*.html
