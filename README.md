...
- **Madagascar (seismic / RSF)** — `sfin`, `sfwindow`, `sfmath`, `sfcat`, `sfattr`, `sfreverse`, `sftransp`, `sfdd`, `sfpad`, `sfdisfil`, `sfheaderwindow`, `sfheadersort`, `sfheadercut`, `sfget`, `sfput`, `sfspike`, `sfcmplx`, `sfreal`/`sfimag`, `sfinterleave`, `sfmask`, `sfclip`, `sfsmooth`, `sfbandpass`, `sfricker`, `sfagc` — key=value parameter style with names + types resolved from each tool's `sf_get*()` calls across `ahay/src/system/{main,generic,seismic}/*.c` (e.g. `Mclip.c clip= value=`, `Msmooth.c rect# diff# box# adj=`, `Mbandpass.c flo= fhi= phase= nplo= nphi=`, `Magc.c rect# repeat=`)
