...
- **Astrometry.net** (astronomical plate-solving aux) — `image2xy` (FITS source extractor), `image2pnm` (raster→PNM), `wcsinfo` (WCS header inspector), `wcs-xy2rd`/`wcs-rd2xy` (pixel↔RA/Dec) — alongside existing `solve-field`/`plot-constellations`; flag tables read from each tool's `print_help()` / `getopt` source at `dstndstn/astrometry.net util/{wcs-xy2rd-main,wcs-rd2xy-main,wcsinfo,image2pnm.py}.c|.py` plus `manpages.debian.org` `image2xy.1`
