...
- **GMT** (Generic Mapping Tools, classic-mode) — 30 binaries: `psxy`, `pscoast`, `grdimage`, `grdmath`, `grdtrack`, `pstext`, `psbasemap`, `psconvert`, `grdfilter`, `grdsample`, `grdcontour`, `xyz2grd`, `grd2xyz`, `nearneighbor`, `blockmean` (shared compdef also serves `blockmedian`/`blockmode`), `mapproject`, `triangulate`, `makecpt`, `grdinfo`, `grdcut`, `grdedit`, `grdgradient`, `grdblend`, `gmtset`, `gmtwhich`, `grdvector`, `gmtselect`, `gmtmath`, `sample1d`, `fitcircle` (alongside existing `gmt` modern-mode wrapper); single-letter option syntax (`-J` projection, `-R` region, `-B` basemap, `-W` pen, `-G` fill, …) + per-program options (e.g. `pscoast -D{full,high,intermediate,low,crude}`, `grdtrack -S{mean,median,mode}` stack reducers, `grdmath`/`gmtmath` 200+ RPN operators each, `grdfilter -F{b,c,g,f,o,m,p,h,l,L,u,U}` filter types, `psconvert -T{b,e,f,j,g,G,m,s,t,F}` output formats, `blockmean -S{m,n,s,w}` statistic, `makecpt -F{R,r,h,c,gray}` CPT format, `grdcut -E{x,y}` 3-D cube slice axis, `grdblend -C{f,l,o,u}` clobber mode, `gmtwhich -G{a,c,l,u}` download destination, `sample1d -F{l,a,c,n,L,A,C,N}` interpolation, `fitcircle -L{1,2}` L1/L2 norm) harvested from https://docs.generic-mapping-tools.org/latest/*.html
