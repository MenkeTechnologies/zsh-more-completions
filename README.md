...
- **Orfeo Toolbox** (Earth observation / remote sensing) — `otbcli_BandMath`, `otbcli_ExtractROI`, `otbcli_ConcatenateImages`, `otbcli_Rasterization`, `otbcli_Segmentation`, `otbcli_OpticalCalibration`, `otbcli_OrthoRectification`, `otbcli_Smoothing`, `otbcli_HaralickTextureExtraction`, `otbcli_ImageClassifier`; per-app parameter trees (including dotted sub-parameters like `-mode.extent.ulx`, `-filter.meanshift.spatialr`, `-acqui.sun.elev`, `-outputs.spacingx`, `-type.gaussian.stdev`, `-parameters.nbbin`) harvested from the OTB CookBook reference at https://www.orfeo-toolbox.org/CookBook/Applications/
