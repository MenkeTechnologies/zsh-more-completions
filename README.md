...
- **Orfeo Toolbox** (Earth observation / remote sensing) — `otbcli_BandMath`, `otbcli_ExtractROI`, `otbcli_ConcatenateImages`, `otbcli_Rasterization`, `otbcli_Segmentation`; per-app parameter trees (including dotted sub-parameters like `-mode.extent.ulx`, `-filter.meanshift.spatialr`, `-mode.vector.fieldname`) harvested from the OTB CookBook reference at https://www.orfeo-toolbox.org/CookBook/Applications/
