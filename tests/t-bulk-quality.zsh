...
@test 'more_src has more than 1700 files' {
    local count=$(ls "$xsrc" | wc -l | tr -d ' ')
    assert "$count" is_greater_than 1700
}
...
@test 'total completion count exceeds 15300' {
    local s=$(ls "$src" | wc -l | tr -d ' ')
    local x=$(ls "$xsrc" | wc -l | tr -d ' ')
    local m=$(ls "$msrc" | wc -l | tr -d ' ')
    local a=$(ls "$asrc" | wc -l | tr -d ' ')
    local o=$(ls "$osrc" | wc -l | tr -d ' ')
    local total=$(( s + x + m + a + o ))
    assert "$total" is_greater_than 15300
}
...
@test 'more than 50% of man_src files reference /usr/share/man - sample 200' {
    ...
    local total=0
    for f in "$msrc"/_*; do
        grep -q '/usr/share/man\|/usr/local/share/man\|/opt/homebrew/share/man\|/Library/.*man\|Autogenerated from man page' "$f" && (( good += 1 )) || true
        (( total += 1 ))
        (( total >= 200 )) && break
    done
    local pct=$(( good * 100 / total ))
