#compdef rst2latex.py
# Documentation: https://man.archlinux.org/search?q=rst2latex.py
...
arguments=(
  ...
  '--pep-base-url[base URL for PEP references]'
  '--pep-file-url-template[template for PEP file part of URL.]'
  '--rfc-references[recognize and link to standalone RFC references (like]'
  '--rfc-base-url[base URL for RFC references]'
  ...
  '--documentclass[specify documentclass. Default is "article".]'
  '--documentoptions[specify document options. Multiple options can be]'
  '--docutils-footnotes[footnotes with numbers/symbols by Docutils. (default)]'
  '--footnote-references[format for footnote references: one of "superscript"]'
  ...
  '--stylesheet-dirs[dir,...\]>]'
  '--latex-preamble[customization by LaTeX code in the preamble. Default:]'
  ...
  '--hyperref-options[additional options to the "hyperref" package]'
  '--compound-enumerators[enable compound enumerators for nested enumerated]'
  '--no-compound-enumerators[disable compound enumerators for nested enumerated]'
  '--section-prefix-for-enumerators[enable section ("." subsection ...) prefixes for]'
  '--compound-enumerators[.Default: disabled.]'
  '--no-section-prefix-for-enumerators[disable section prefixes for compound enumerators.]'
  '--section-enumerator-separator[set the separator between section number and]'
  '--literal-block-env[when possibile, use the specified environment for]'
  '--use-verbatim-when-possible[when possibile, use verbatim for literal-blocks.]'
  '--table-style[table style. "standard" with horizontal and vertical]'
  '--graphicx-option[laTeX graphicx package option. Possible values are]'
  '--font-encoding[laTeX font encoding. Possible values are "", "T1"]'
  '--reference-label[per default the latex-writer puts the reference title]'
  '--use-bibtex[specify style and database for bibtex, for example "--]'
  '*:filename:_files'
)
