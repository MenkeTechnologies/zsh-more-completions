#compdef rst2s5.py
# Documentation: https://man.archlinux.org/search?q=rst2s5.py
...
arguments=(
  ...
  '--template[specify the template file (UTF-8 encoded). Default is]'
  '--stylesheet[URL,...\]>]'
  ...
  '--stylesheet-dirs[dir,...\]>]'
  '--initial-header-level[specify the initial header level. Default is 1 for]'
  '--field-name-limit[specify the maximum width (in characters) for one-]'
  '--option-limit[specify the maximum width (in characters) for options]'
  ...
  '--no-compact-field-lists[disable compact simple field lists.]'
  '--table-style[added to standard table classes. Defined styles:]'
  '--math-output[math output format, one of "MathML", "HTML", "MathJax"]'
  '--no-xml-declaration[omit the XML declaration. Use with caution.]'
  '--cloak-email-addresses[obfuscate email addresses to confuse harvesters while]'
  '--theme[specify an installed S5 theme by name. Overrides]'
  '--theme-url[. The default theme name is "default".]'
  '--theme-url[specify an S5 theme URL. The destination file (output]'
  '--overwrite-theme-files[allow existing theme files in the ``ui/<theme>``]'
  '--keep-theme-files[keep existing theme files in the ``ui/<theme>``]'
  '--view-mode[set the initial view mode to "slideshow" \[default\] or]'
